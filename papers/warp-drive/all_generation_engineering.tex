...
\subsection{Near-transparency with two-prime muting}

Numerical computation reveals that for any pair of primes
$\{p, q\}$, the modified corrections $C_\mu'$ and $C_\tau'$
are both very large (Table~\ref{tab:muting}), meaning
the \emph{inverse} correction is very small.
In the ``inverted'' frame (where the original corrections
were defined), two-prime muting drives the effective correction
toward $-1$ (near-zero absolute value).

Physically: muting two primes makes all matter approximately
\emph{transparent} to the arithmetic vacuum corrections,
as if the particles temporarily decouple from the Spec(Z)
structure.
