...
\subsection{既存実験との整合性}

\textbf{プラトー発見は既存実験と矛盾しない．}

この点は慎重に整理する必要がある．一見「標準 QED の精密検証と
衝突する」と考えたくなるが，実際にはそうではない．主要な既存測定を
列挙すると：

\begin{itemize}
\item \textbf{Lamoreaux (1997), Mohideen (1998), Bressi (2002),
Decca (2003+), Munday (2009)} その他の Casimir 測定：
全て\textbf{DD (金属--金属) 境界}，距離 100\,nm--数\,$\mu$m．
これらは DN レジームを測定していないので，DN プラトーとは
論理的に独立
\item \textbf{原子精密 QED 試験}（電子 $g-2$，水素 Lamb shift，
微細構造定数）：\textbf{境界条件のない自由真空}での効果．
DN プラトーは境界を持つ真空特有の現象なので影響しない
\item \textbf{ミューオン $g-2$，ハドロン寄与}：同上，境界なし
\item \textbf{Eöt-Wash トーション天秤}（Adelberger ら）：
$\sim 50\,\mu$m までの重力逆二乗則検証で null．ただし
\textbf{通常物質は DN 境界を作らない}ので，プラトー効果は
トーション天秤で検出されない．論理的に排除されない
\item \textbf{宇宙論的真空エネルギー}$\rho_\Lambda \sim 10^{-9}$\,J/m³：
これが唯一の実質的制約．ただし\textbf{プラトー密度 $=\rho_\Lambda$}
と同定する可能性こそが Wright Brothers 仮説の核心であり，
制約どころか\textbf{ダークエネルギーの説明}となる
（詳細は \texttt{dark\_energy\_plateau\_ja.tex} 参照）
\end{itemize}

したがって「事前確率が低い」の正確な理由は\textbf{実験的矛盾では
なく理論的保守性}である——標準 QED の mode-sum 描像は一般に
信頼されているが，DN レジームでの実験的検証は存在しない．これは
\textbf{慣性による保守性であって物理的排除ではない}．

\subsection{正直な期待値}

プラトー発見は「未測定領域への合理的探索」と位置付けるのが
正確．既存実験と無矛盾，独立な宇宙論的動機（ダークエネルギー），
位相的保護という数学的根拠（$K_1(\mathbb Z[1/2])$）がある．
「低確率・高リターン」というより\textbf{「誰も見ていないから
答えを持っている人がいない」領域}．

いずれにしても，Phase A を実行する合理性は複層：
\begin{itemize}
\item プラトー発見 → 革命的．ダークエネルギー問題の解決候補
\item 傾き $-3$ 確認 → 標準 QED の DN 版初検証．これ自体 APL レベル
\item 符号反転（測定 1--6）の結果は別途確実に得られる
\item 厚さシリーズ自体が RF 工学的にも新規データ
\end{itemize}

\textbf{期待値 = 確実な成果 $+$ 合理的確率での革命}．これが Phase A を
実行する理由．
