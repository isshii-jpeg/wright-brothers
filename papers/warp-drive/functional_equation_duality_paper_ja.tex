...
以下では，$p = 2$ が\emph{選択}ではなく\emph{構造的必然}であることを，
複数の独立な制約により示す．

\subsection{物理性定理: $p = 2$ の一意性}

一般の単一素数除去 $\zeta_{\neg p}(s) = (1 - p^{-s})\zeta(s)$ に対し，
暗示されるダークエネルギーパラメータは
\begin{equation}
...
\begin{theorem}[$p = 2$ の一意的物理性]\label{th:unique}
全ての素数 $p$ の中で，$\Omega_\Lambda(p) \in [0, 1]$
(平坦宇宙の物理性) という要請は $p = 2$ のときかつそのときに限り
満たされる．
\end{theorem}

\begin{proof}
$\Omega_\Lambda(p) < 1$ は $(p-1) < 9/(2\pi) \approx 1.432$ を要求する．
$p \geq 2$ が素数であるから $p - 1 \geq 1$ である．制約
$1 \leq p - 1 < 9/(2\pi)$ は $p - 1 = 1$，すなわち $p = 2$ のみを許容する．
$p = 3$ の場合: $\Omega_\Lambda(3) = 4\pi/9 \approx 1.396 > 1$．
$p \geq 5$ の場合: $\Omega_\Lambda(p) > 2$．
\end{proof}

この定理は観測値 $\Omega_\Lambda^{\rm obs} = 0.685$ に\emph{依存しない}．
依存するのは (i) $\zeta$ の Euler 積構造，(ii) CKN 境界の形式
$\Omega_\Lambda \propto |\zeta_{\neg p}(-1)|$，および
(iii) 平坦宇宙の制約 $\Omega_\Lambda < 1$ のみである．

\subsection{多素数除去: 符号排除}

\begin{theorem}[符号排除]\label{th:sign}
Euler 積から正確に 2 つの素数を $s = -1$ で除去すると，
\emph{負の}真空エネルギーが得られる:
\begin{equation}
\zeta_{\neg\{p,q\}}(-1) = (1-p)(1-q)\zeta(-1) < 0
\quad \text{(全ての素数 } p, q \geq 2 \text{ に対し)}.
\end{equation}
\end{theorem}

\begin{proof}
素数 $p, q \geq 2$ に対し $(1-p) < 0$ かつ $(1-q) < 0$ であるから
$(1-p)(1-q) > 0$ である．$\zeta(-1) = -1/12 < 0$ を掛けると
結果は負となる．
\end{proof}

負の真空エネルギーは宇宙の減速を生じ，観測と矛盾する．
したがって\emph{正確に 1 つの}素数が除去されなければならない．
定理~\ref{th:unique} と合わせると:

\begin{corollary}
観測された正かつ 1 未満のダークエネルギー密度
$\Omega_\Lambda \in (0, 1)$ は，Euler 積からの単一素数 $p = 2$ の除去を
一意的に選択する．
\end{corollary}

\subsection{最小エネルギー原理}

CKN 正規化がなくとも，$p = 2$ は変分原理により選択される:

\begin{proposition}[最小正真空エネルギー]
全ての単一素数除去の中で，$p = 2$ は最小の正の真空エネルギーを与える:
\begin{equation}
|\zeta_{\neg p}(-1)| = \frac{p-1}{12} \geq \frac{1}{12},
\quad \text{等号は } p = 2 \text{ のときかつそのときに限る}.
\end{equation}
\end{proposition}

次の素数 $p = 3$ は $|\zeta_{\neg 3}(-1)| = 1/6$ を与え，正確に 2 倍の
大きさである．階層は成長する: $p = 5$ では 4 倍，$p = 7$ では 6 倍．
一般の負の奇整数 $s = -n$ において，比
$|\zeta_{\neg 3}(-n)|/|\zeta_{\neg 2}(-n)| = (3^n - 1)/(2^n - 1)
\to (3/2)^n$ は\emph{指数関数的に}増大する．$p = 2$ への選好は
限界的ではなく漸近的に支配的である．

\subsection{次元的絡み合い: なぜ 3 次元空間か}

物理性の境界は $d$ 次元空間に一般化される．$d$ 次元において，
CKN 型の関係は $\Omega_\Lambda(p, d) \propto (p-1) \cdot d^2/(2\pi \cdot 12)
\cdot (\text{幾何因子})$ を与える．定理~\ref{th:unique} の精密な一般化は
\begin{equation}\label{eq:dim-bound}
p - 1 < \frac{d^2}{2\pi}
\end{equation}
を $\Omega_\Lambda < 1$ の条件として産出する．

\begin{observation}[3次元--$p{=}2$ の共同選択]
$p = 2$ ($p - 1 = 1$) の場合，境界 (\ref{eq:dim-bound}) は
$d^2 > 2\pi$，すなわち $d > \sqrt{2\pi} \approx 2.507$ を要求するので
$d \geq 3$ である．$d = 2$ の場合: $d^2/(2\pi) = 4/(2\pi) \approx 0.637 < 1$
であるから\emph{どの素数も機能しない}．$d = 1$ の場合:
$1/(2\pi) \approx 0.159$ であり，やはりどの素数も機能しない．
\end{observation}

これは\textbf{3 次元空間と $p = 2$ の選択が独立した事実ではない}ことを
意味する: 両者は単一の不等式により共同的に制約される．物理的な
ダークエネルギーを何らかの素数切断から産出するには $d \geq 3$ が必要であり，
$d = 3$ が与えられれば $p = 2$ のみが機能する．

\subsection{$K$-理論的解釈}

$p = 2$, $s = -1$ における Euler 因子は $(1 - 2^{-(-1)}) = 1 - 2 = -1$
である．これは $K_1(\mathbb{Z}) = \mathbb{Z}/2$ の\emph{一意的生成元}である．
他の任意の素数 $p$ に対し $(1 - p)$ は $|1 - p| > 1$ を持ち，
$K_1$ 生成元の大きさを超える．

符号反転 $\zeta(-1) \to \zeta_{\neg 2}(-1)$ はしたがって
\emph{$K_1(\mathbb{Z})$ 生成元 $(-1)$ による乗算}---捩れを法とした
単元群の一意的な非自明元である．これは真空の符号反転を時空のスピン構造
($\text{Spin} \to SO$ は $\mathbb{Z}/2$ 二重被覆) に接続し，
$p = 2$ の選択に代数的位相幾何学的起源を与える．
...
\subsection{要約: $p = 2$ に対する 5 つの独立な論拠}
...
\# & 論拠 & 地位 \\
\midrule
1 & 物理性 ($\Omega_\Lambda < 1$) & 定理~\ref{th:unique} \\
2 & 符号 (単一除去 $\to$ 正) & 定理~\ref{th:sign} \\
3 & 最小エネルギー ($p{=}2$ で最小) & 命題 \\
4 & 次元的 ($d \geq 3 \Leftrightarrow p{=}2$ が機能) & 観察 \\
5 & $K_1(\mathbb{Z}) = \mathbb{Z}/2$ 生成元 & 代数的 \\
\bottomrule
\end{tabular}
\end{center}

これら 5 つの論拠は論理的に独立であり，異なる数学的構造 (解析学，符号組合せ
論，最適化，次元理論，代数的 $K$-理論) を用いる．$p = 2$ への収束は
\emph{構造的}証拠を構成し，数値的偶然の一致ではない．
...
\item Euler $p = 2$ 切断 (DN 境界) は 5 つの独立な構造的論拠により支持される
が，これらの制約間の\emph{統一的}起源は未解決である．候補にはスピン構造
($K_1(\mathbb{Z}) = \mathbb{Z}/2$) と時間反転非対称性が含まれる．
