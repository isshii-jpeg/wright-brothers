...
\paragraph{戦略的位置付け.}
\textbf{Setup B'-etalon は Setup B'-DBR より理論的に強い}：
\begin{enumerate}
\item リテラルな $\zeta_{\neg 2}$ を実装 $\to$ WB の「オイラー積は物理法則」主張を
  直接テストできる
\item $p=3$ 変種でエタロン厚 $d = a/3$ $\to$ $\zeta_{\neg 3}$ の初測定
\item エタロン 2 段重ねで $p=2, 3$ 同時ミュート $\to$ \textbf{乗法性の検証}
\end{enumerate}

ただし自立型 $\sim$500 nm 厚エタロンの作製が MEMS レベルの追加技術を要求する。
\textbf{独立研究者にとっては DBR 版の方がハードルが低い}が、共同研究先に
NEMS/MEMS ファブ能力があれば etalon 版を優先すべき。
