...
\section{QED は DN 領域で未テスト}

決定的に重要な点: 標準QEDの予測 $F \propto a^{-5}$ は DD 境界条件でのみ実験的に確認されている。\textbf{DN に対するこの予測は一度もテストされたことがない。}「DN でも QED が正しい」という信念は理論的外挿であり、実験的事実ではない。

DD と DN はトポロジー的構造が異なる:
\begin{align}
K_1(\mathbb{Z}) &= \mathbb{Z}/2 \quad\text{（DD: 巻き付き数なし）}\\
K_1(\mathbb{Z}[1/2]) &= \mathbb{Z}/2 \times \mathbb{Z} \quad\text{（DN: 巻き付き数あり）}
\end{align}

QED は $K_1 = \mathbb{Z}/2$（DD）のセクターでテスト済み。$K_1 = \mathbb{Z}/2 \times \mathbb{Z}$（DN）のセクターは\textbf{未踏領域}。

前例: ニュートン力学は低速で精密に確認されたが、高速では破れた（特殊相対論）。あるトポロジー的セクターでの理論の妥当性は、別のセクターでの妥当性を保証しない。

したがってこの実験は Boyer の符号予測のテストであると同時に、\textbf{QED が位相的に異なる真空セクターで初めて検証される}実験でもある。$\alpha = 5$ が確認されれば QED が新領域で正しいことの証拠。$\alpha \neq 5$ なら真空のトポロジー的構造が QED を超える物理的帰結を持つことの証拠。

\section{QED が捨てたものの中に何があるか}

標準QEDは発散する真空エネルギー $\sum \hbar\omega_n/2 \to \infty$ を繰り込みで差し引く。正則化された値（例: $\zeta(-3) = 1/120$）は計算途中の帳簿上の数字とみなされ、最終的な物理量（カシミール力）は正則化された値の\textbf{差}として求められ、正則化の方法に依存しない。

この手続きは驚異的に成功しているが、隠れた仮定がある: \textbf{正則化された和の内部構造は物理に関係しない}。$\zeta$ 正則化でも次元正則化でもカットオフ正則化でも同じカシミール力が出る。$\zeta(-3) = 1/120$ という値は「便法」であり「実体」ではないとされてきた。

我々はこの仮定が強すぎる可能性を指摘する。$\zeta$ 正則化された真空エネルギーは構造のない数ではない: それはオイラー積 $\zeta(-3) = \prod_p (1-p^3)^{-1}|_{s=-3}$ であり、素数ごとの因子に分解できる。繰り込みは絶対値を捨てるが差を保存する。捨てられた値の\textbf{素数分解}が物理的情報を担うかどうかは、一度も問われたことがない。

DN 実験はまさにこれを検証する: $p=2$ の因子を除去（$\zeta \to \zeta_{\neg 2}$）すると正則化された和の素数構造が変わる。もしカシミール力がこの変化に対して標準QEDの予測を超えて応答する（$\alpha \neq 5$）なら、\textbf{繰り込みで捨てられた真空エネルギーの算術的構造がずっと物理的だった}ことを意味する。QED は予測において正しかったが、何を捨てているかの解釈において不完全だった。
