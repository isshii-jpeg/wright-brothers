...
\begin{abstract}
We develop four approaches to manipulating quantum vacuum energy
...
the Euler product as a ``arithmetic drum.''
We prove that rapid switching between muted and unmuted states yields
exactly zero net energy (vacuum Carnot theorem), confirming energy
conservation.
...
vs.\ 1:3)---is achievable with existing acoustic resonator technology
at a cost of approximately \$2{,}200.
\end{abstract}
...
Four distinct approaches to vacuum manipulation were developed,
each with different strengths: the spectral action approach provides
conceptual clarity, $\beta$-pumping offers immediate experimental
access, Euler product amplification gives the largest potential effect,
and inverse spectral design provides the cleanest implementation.

All four converge on a single decisive experiment: measuring the
Casimir energy ratio in a 2-prime-sieved acoustic cavity.
The predicted ratio (182:1 vs.\ 1:3) is large enough for unambiguous
determination with modest equipment (\$2{,}200).
...
vacuum, opening a new chapter in vacuum engineering.
