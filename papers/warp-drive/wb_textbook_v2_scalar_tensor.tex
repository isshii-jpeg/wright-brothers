...
本章では、Wright Brothers 理論の中核をなす数学的結果
...
本節が本章の核心であり、Wright Brothers理論全体の数学的基盤を与える。
...
\begin{keyresult}[WB スカラー・テンソル作用（Jordan frame）]
\begin{equation}
\boxed{
S = \int d^4x \sqrt{-g}\left[
  \frac{\cos(\varphi)}{16\pi G}\,R
  - \frac{\Lambda_{\mathrm{WB}}\cos(2\varphi)}{8\pi G}
...
\paragraph{第2項：$-\Lambda_{\mathrm{WB}}\cos(2\varphi) / (8\pi G)$}

これは$a_0$項から導かれる。$\Lambda_{\mathrm{WB}}$は裸の宇宙定数スケールである。
...
を代入すると：
\begin{align}
-\frac{\Lambda_{\mathrm{WB}}}{8\pi G}\cos(2\varphi)
&= -\frac{\Lambda_{\mathrm{WB}}}{8\pi G}
  + \frac{\Lambda_{\mathrm{WB}}}{4\pi G}\sin^2(\varphi)
\end{align}
第1項は定数（通常の宇宙定数）であり、第2項がスカラー場のポテンシャルを与える：

\begin{keyresult}[ポテンシャルの導出]
\begin{equation}
V(\varphi) = \frac{\Lambda_{\mathrm{WB}}}{4\pi G}\sin^2(\varphi)
  = \delta\,\sin^2(\varphi)
\label{eq:potential}
\end{equation}
ここで
\begin{equation}
\delta \equiv \frac{\Lambda_{\mathrm{WB}}}{4\pi G}
...
\boxed{
\cos(\varphi)\,G_{\mu\nu}
+ g_{\mu\nu}\,\Lambda_{\mathrm{WB}}\cos(2\varphi)
+ g_{\mu\nu}\square\cos(\varphi)
- \nabla_\mu\nabla_\nu\cos(\varphi)
...
+ \Lambda_{\mathrm{WB}}\cos(2\varphi)
...
\begin{equation}
3H^2\cos(\varphi) = \Lambda_{\mathrm{WB}}\cos(2\varphi)
...
WBスカラー・テンソル理論の修正Friedmann方程式と修正Klein--Gordon方程式は、
...
\textbf{Step 2.}
式\eqref{eq:vacuum-friedmann}より$3H^2 = \Lambda_{\mathrm{WB}}\cos(2\varphi)/\cos(\varphi)$を
\eqref{eq:step1}に代入する：
\begin{equation}
2\delta\cos(\varphi) = \frac{\Lambda_{\mathrm{WB}}\cos(2\varphi)}{4\pi G\cos(\varphi)}
\end{equation}

\textbf{Step 3.}
$\delta = \Lambda_{\mathrm{WB}}/(4\pi G)$を用いて整理する：
\begin{align}
2 \cdot \frac{\Lambda_{\mathrm{WB}}}{4\pi G}\cos(\varphi)
&= \frac{\Lambda_{\mathrm{WB}}}{4\pi G}\cdot\frac{\cos(2\varphi)}{\cos(\varphi)} \\
...
したがってWB理論は$\varphi=0$相においてGRの全ての成功を自動的に継承する。
\end{remark}
...
-3H^2 = \Lambda_{\mathrm{WB}}
\end{equation}
となる。$\Lambda_{\mathrm{WB}} > 0$であるから$H^2 < 0$が要求される。
...
本章では、WB理論がPPN制約を\textbf{自動的に}満たすことを証明し、
...
\end{equation}
で与えられる。WB理論では$F(\varphi) = \cos(\varphi)$であり、
標準的な運動エネルギーを持つので：

\begin{keyresult}[WB理論のPPNパラメータ]
\begin{equation}
...
\label{thm:ppn-auto}
WBスカラー・テンソル理論の真空量子化された解$\varphi = 0$および$\varphi = \pi$
...
WB理論では事情が全く異なる。Berry位相場$\varphi$は$0$か$\pi$の
...
WB理論の位置づけを明確にするため、主要なスカラー・テンソル理論
および一般相対論との系統的比較を表\ref{tab:theory-comparison}に示す。

\begin{table}[htbp]
\centering
\caption{WBスカラー・テンソル理論と他理論の比較}
\label{tab:theory-comparison}
\renewcommand{\arraystretch}{1.5}
\begin{tabular}{@{}lcccc@{}}
\toprule
& \textbf{GR} & \textbf{Brans--Dicke} & \textbf{$f(R)$理論} & \textbf{WB理論} \\
\midrule
$\Lambda$（宇宙定数）
...
WB理論では$\Omega_\Lambda = 2\pi/9$が第一原理から導出される
（Part II、第4章）。

\paragraph{スカラー場の値}
Brans--Dicke理論と$f(R)$理論では、スカラー場は任意の実数値を取り得る。
WB理論ではBerry位相場$\varphi$は真空量子化（定理\ref{thm:vacuum-quantization}）
...
WB理論では$\gamma = 1$が三角関数の性質により厳密に成立する。
...
安定性やゴーストの問題を伴う。
WB理論では$\varphi = \pi$により$G_{\mathrm{eff}} = -G$が実現されるが、
...
実質的に無限個のパラメータを持つ。
WB理論は$G$がスペクトル作用から、$\Lambda$が$\Omega_\Lambda = 2\pi/9$から
...
\end{enumerate}
これら4つの結果は、WB理論が一般相対論の全ての実験的成功を継承しつつ、
