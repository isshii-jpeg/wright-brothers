...
\subsection{重要な区別：DBR 版 vs Fabry-Perot エタロン版}

\textbf{本ガイドの Setup B' には 2 つの物理実装がある}：

\begin{enumerate}
\item \textbf{Setup B'-DBR}：多層膜 DBR を PMC 等価境界として使う。
  実装する関数は Hurwitz ゼータ $\zeta(s, 1/2)$（半整数モード）。
  長さスケール変換により $\zeta_{\neg 2}$ と物理的に等価だが、
  $p=3$ 以降への一般化は不可能。
\item \textbf{Setup B'-etalon}：Fabry-Perot エタロン（厚さ $a/2$）を壁に使う。
  実装する関数は\textbf{リテラルな} $\zeta_{\neg 2}(s) = \sum_{n \text{ odd}} n^{-s}$。
  $p=3$ 変種はエタロン厚 $d = a/3$ で実現可能、$p=2, 3$ 同時ミュートも可。
\end{enumerate}
...
\toprule
& \textbf{Setup B'-DBR} & \textbf{Setup B'-etalon} \\
\midrule
実装する関数 & Hurwitz $\zeta(s, 1/2)$ & \textbf{リテラル $\zeta_{\neg 2}(s)$} \\
Boyer 比 ($s=-3$) & $-7/8$ & \textbf{$-8$} \\
$p=3$ 一般化 & 不可能 & \textbf{$d=a/3$ で可能} \\
乗法性検証 & 不可能 & \textbf{エタロン 2 段重ね} \\
部品調達 & Layertec DBR ¥400k & \textbf{商用エタロン ¥50--200k} \\
実装難度 & 中（DBR 注文のみ） & 高（自立エタロン製作） \\
SN (at $a=1\,\mu$m) & $\sim 800$ (比 7/8 相当) & \textbf{$\sim 9000$} (比 8 倍) \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{独立研究者向け推奨（訂正版）}：
\begin{itemize}
\item \textbf{第 1 選択：Setup B'-etalon}（広帯域で物理的に強い、
  商用 LightMachinery air-spaced 版 ¥300--800k、または NEMS 共同研究）
\item 第 2 選択：Setup B'-DBR（狭帯域だが注文プロセスが簡単、partial 効果で
  Akkermans 原理検証まで）
\item 理想：両方を並行で走らせて、互いに独立な検証として論文化
\end{itemize}

\textbf{\emph{earlier 版からの重要な訂正}}：当初「DBR が簡単なので第 1 選択」としていたが、
これは「注文プロセスが外注できる」という浅い意味に過ぎない。物理的には以下 2 つの
DBR 問題により、\textbf{DBR は Boyer 完全検証には不十分}である：

\begin{enumerate}
\item \textbf{狭帯域問題}：DBR の stop band は $\Delta\lambda/\lambda \sim 20\%$ で、
  Casimir 関連スペクトル（DC$\sim$UV）の狭いスライスにしか効かない。単一 DBR で
  期待される効果は $-7/8$ ではなく数 \% 程度
\item \textbf{反射位相問題}：同じ反射率 99\% でも、top 層が高屈折率か低屈折率かで
  $\arg(r)$ が 0（PMC 等価）か $\pi$（PEC 等価）に分かれる。通常の商用 DBR は後者で、
  Boyer に必要な前者を得るには発注時の明示指定と受領後の楕円偏光法による検証が必要
\end{enumerate}

詳細は \texttt{experimental\_guide\_antigravity\_ja.pdf} §3 の末尾を参照。
...
\subsection{B: DBR 多層膜（Setup B'-DBR 用）}

\begin{warnbox}
\textbf{DBR は「注文するだけ」ではない}。以下 4 段階の作業と、2 つの物理的限界
（狭帯域・位相）がある。Setup B'-etalon の方が物理的には本命で、DBR は原理検証用
として位置付けるべき。
\end{warnbox}

\paragraph{DBR 準備の 4 段階.}

\begin{enumerate}
\item \textbf{設計（1--2 週間、熟練者前提）}
  \begin{itemize}
  \item 中心波長 $\lambda_0$ の選定（Casimir 距離に対して $\lambda_0 \sim 2L$ が主寄与帯）
  \item 層数（15--30 ペア）、材料（SiO$_2$/Ta$_2$O$_5$ 等）決定
  \item \textbf{位相設計}：top 層を\emph{高屈折率}にして $\arg(r) \approx 0$ を保証
  \item Transfer matrix シミュレーション（Akkermans 式 10.83--10.95）で検証
  \item Akkermans 公式 (10.40) を逆解いて目標 $\delta(\omega)$ と比較
  \end{itemize}

\item \textbf{発注（2--4 週間 + 製作 6--12 週間）}
  \begin{itemize}
  \item 仕様書に「Top layer: high-index (Ta$_2$O$_5$), for $\arg(r) \approx 0$
    at $\lambda_0$」を\textbf{明示}
  \item 英語でベンダーと物理的根拠をやり取りできる必要あり
  \item 標準 supermirror は使えない（反射位相は top 層次第）
  \end{itemize}

\item \textbf{受領後の評価（1--2 週間）}
  \begin{itemize}
  \item \textbf{分光光度計}：$|r(\omega)|^2$ の帯域測定（大学共用設備で OK）
  \item \textbf{楕円偏光法}：$\arg(r)$ の絶対値測定（ここが難しい、
    校正済み楕円偏光計があるラボは限られる）
  \item \textbf{AFM 粗さ測定}：$< 1$ nm RMS 確認
  \item \textbf{Zygo 干渉計}：平坦度 $\lambda/10$ 確認
  \end{itemize}

\item \textbf{Casimir 装置への組み込み（1--2 週間）}
  \begin{itemize}
  \item 平行度 $\mu$rad レベルの調整
  \item 温度ドリフトで DBR が熱応力で反るのを抑制
  \item Kelvin probe による patch potential 補正
  \end{itemize}
\end{enumerate}
...
\textbf{(a) 狭帯域問題}：
単一 DBR の stop band は $\Delta\lambda/\lambda \sim 20\%$ 程度。Casimir 積分の
狭いスライスにしか影響しない。
\begin{itemize}
\item 単一 DBR の期待効果：$\sim$ 数 \% 比（$-7/8$ ではない）
\item Chirped DBR（50--100 層）：帯域広く取れるが ¥1--3M、設計複雑
\item 狭帯域版は「Akkermans 公式の原理検証」には有効、Boyer 完全検証には不十分
\end{itemize}

\textbf{(b) 反射位相問題}：
商用ベンダーの datasheet は $|r|^2$ と帯域しか書かないが、実際には：
\[
  \arg(r)_{\text{top = high-}n} \approx 0 \quad \text{vs} \quad
  \arg(r)_{\text{top = low-}n} \approx \pi
\]
\textbf{通常の商用品は後者（PEC 等価、Boyer に不適）}。前者（PMC 等価）を得るには：
\begin{itemize}
\item 発注時に「Top layer: high-index」と明示
\item 受領後に楕円偏光計で実測して $\arg(r) \approx 0$ を確認
\item いずれも熟練光学者のサポートが必要
\end{itemize}

\paragraph{調達先}
\begin{itemize}
\item \textbf{Layertec GmbH}（独）：custom DBR、¥300--500k、納期 6--8 週、phase 指定可
\item \textbf{Advanced Thin Films}（米）：IBS、¥500k--1M、高品質
\item \textbf{Chirped broadband DBR}：LIGO/KAGRA 系と共同、または Layertec custom、¥1--3M
\item \textbf{Newport / MKS standard mirrors}：\emph{非推奨}（phase 保証なし）
\end{itemize}

\subsection{B': Fabry-Perot エタロン（Setup B'-etalon 用）}

\begin{itemize}
\item \textbf{構造}：2 枚の半透明ミラー（$R \approx 0.77$）+ 厚さ $d = a/2$ のスペーサー
\item \textbf{Finesse 目標}：$F \geq 30$（偶モード透過率 $> 0.97$、奇モード透過率 $< 0.04$）
\item \textbf{厚さ精度}：$\Delta d / d < 10\%$（FSR のズレを偶モードのバンド幅以内に抑える）
\item \textbf{全体厚さ}：カシミール実験では薄基板が必須
  \begin{itemize}
  \item 通常の商用エタロン（mm 厚ガラス基板）は使えない
  \item 解決策 1：Si$_3$N$_4$ メンブレン（$\sim 50$ nm 厚）上に成膜
  \item 解決策 2：自立型多層膜エタロン（技術的に困難、研究レベル）
  \end{itemize}
\item \textbf{調達先}：
  \begin{itemize}
  \item \textbf{Thorlabs}：standard etalon（mm 厚基板、¥50--150k）、カシミール用には
    基板除去または薄膜化が必要
  \item \textbf{Edmund Optics}：laser etalons（同上）
  \item \textbf{NEMS/MEMS 共同研究先での自作}：Si$_3$N$_4$ メンブレン + 多層膜、
    \textbf{これが推奨経路}
  \item \textbf{LightMachinery}（加）：custom air-spaced etalons、厚さ調整可能、
    $\sim$ ¥300--800k
  \end{itemize}
\end{itemize}

\textbf{代表寸法（$a = 1\,\mu$m の場合）}：
\begin{itemize}
\item エタロン厚 $d = 500$ nm
\item 基本モード $f_1 = 150$ THz（$\lambda_1 = 2\,\mu$m、近 IR）
\item Finesse $F = 30$ $\to$ 偶モード $T \approx 1$、奇モード $T \approx 0.032$
\item 期待信号：$|P_{\rm odd}/P_{\rm std}| \approx 8$、符号反転、SN $\sim 9000$
\end{itemize}
...
\textbf{earlier 版からの訂正}：当初 A-DBR 版を第 1 選択としていたが、物理的な
限界（狭帯域・位相問題）を考慮すると、A-etalon 版を第 1 選択にすべき。
A-DBR 版は「Akkermans 原理検証」としての位置付けに変更。

\paragraph{A-etalon 版（第 1 選択、物理的に強い）.}
\begin{center}
\begin{tabular}{lr}
\toprule
項目 & 金額 \\
\midrule
商用 air-spaced エタロン（LightMachinery） & ¥300--800k \\
Au 球プローブ + 参照 Au プレート & ¥150k \\
Kelvin 補正回路 & ¥100k \\
消耗品 & ¥100k \\
旅費・滞在費（3 月、海外想定） & ¥300k \\
\midrule
\textbf{合計} & \textbf{¥950k--1.45M} \\
\bottomrule
\end{tabular}
\end{center}

もし Si$_3$N$_4$ メンブレン基板版を自作する場合：
\begin{center}
\begin{tabular}{lr}
\toprule
項目 & 金額 \\
\midrule
Si$_3$N$_4$ メンブレン基板 + 多層膜成膜（MEMS 委託） & ¥500k--1.5M \\
その他（上記同じ） & ¥650k \\
\midrule
\textbf{合計} & \textbf{¥1.15M--2.15M} \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{A-DBR 版（第 2 選択、Akkermans 原理検証用）.}
\begin{center}
\begin{tabular}{lr}
\toprule
項目 & 金額 \\
\midrule
Phase 指定 DBR（Layertec custom、top 層高 $n$） & ¥400--600k \\
楕円偏光測定（共同研究先で借用 or 委託） & ¥100--300k \\
Au 球プローブ + 参照 Au プレート & ¥150k \\
Kelvin 補正回路 & ¥100k \\
消耗品 & ¥100k \\
旅費・滞在費 & ¥300k \\
\midrule
\textbf{合計} & \textbf{¥1.15M--1.45M} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{注意}：A-DBR 版は単一 DBR の狭帯域のため、期待される効果は $\sim -7/8$ ではなく
\textbf{数 \% 比}。Boyer 完全検証ではなく、Akkermans 公式の原理検証として位置付ける。

\paragraph{A-DBR chirped 版（第 3 選択、高コスト）.}
Chirped broadband DBR（50--100 層）を LIGO 共同 or Layertec custom で発注。
DBR 単体で ¥1--3M。これなら広帯域 Boyer 比 $-7/8$ に近づくが、独立研究者の
初期選択としては非現実的。

\paragraph{前提}：
AFM、真空チャンバー、ピエゾ、ロックイン、振動絶縁を全て共同研究先で借用。
A-etalon 版の場合、共同研究先に NEMS/MEMS ファブ能力（または MEMS ファウンドリへの
アクセス）が必要。ただし商用 air-spaced エタロンを使う場合は NEMS 不要で、
Casimir 実験設備だけあれば良い。
