...
\textbf{独立研究者向け推奨}：
\begin{itemize}
\item \textbf{第 1 選択}：Setup B'-DBR（実装が簡単、DBR は商用注文で完結）
\item \textbf{第 2 選択（理論的に強い）}：Setup B'-etalon（ただし自立エタロン or
  Si$_3$N$_4$ メンブレン上エタロンの作製が必要、MEMS/NEMS ファブ能力のある
  共同研究先が必須）
\item \textbf{理想}：両方を並行で走らせて、互いに独立な検証として論文化
\end{itemize}

詳細なエタロン版仕様は §5 の続きと §6 を参照。
...
\paragraph{A-etalon 版（理論的に強い、MEMS/NEMS ファブ要）.}
\begin{center}
\begin{tabular}{lr}
\toprule
項目 & 金額 \\
\midrule
商用エタロン（LightMachinery） & ¥300--800k \\
または Si$_3$N$_4$ 基板上自作（MEMS 委託） & ¥500k--1.5M \\
Au 球プローブ + 参照 Au プレート & ¥150k \\
Kelvin 補正回路 & ¥100k \\
消耗品 & ¥100k \\
旅費・滞在費 & ¥300k \\
\midrule
\textbf{合計（商用エタロン使用時）} & \textbf{¥950k--1.45M} \\
\textbf{合計（自作時）} & \textbf{¥1.15M--2.15M} \\
\bottomrule
\end{tabular}
\end{center}
