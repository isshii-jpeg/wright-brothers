...
Mandelbrot Ape (MA) 理論では、この値を$d=4$と$\cd=3$のみから
...
\end{equation}
MA 理論では時間方向に\textbf{Dirichlet-Neumann (DN) 混合境界条件}を課す：
\begin{itemize}
...
\end{itemize}
MA 理論では、物理定数がこれらの CM 値から決まると主張する。
\end{hsbox}
...
ここに MA 理論の二つの入力$d$と$\cd$が再び登場する：
\begin{equation}
j(i) = 1728 = 12^3 = (d \times \cd)^3 = (4 \times 3)^3
\end{equation}
これは偶然ではない。$12 = d \times \cd$は MA 理論の基本スケールであり、
$\zeta(-1) = -1/12$にも$j(i) = 12^3$にも現れる。

\begin{hsbox}[高校生ノート：12 という数]
$12 = 4 \times 3$という数は MA 理論のいたるところに現れる：
\begin{itemize}[nosep]
...
$12 = d \times \cd = 4 \times 3$ は前章までに確立された MA 理論の基本スケールである。
...
MA 理論は$d = 4$（時空次元）と$\cd = 3$（$\Spec(\Z)$のコホモロジー次元）
