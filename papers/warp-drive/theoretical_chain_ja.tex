...
\textit{(i) 構造的並行性。}
$F$と$\mathcal{A}_{\mathrm{BC}}$はいずれも
類似した構造的性質（有限次元表現、
数体に関連する自己同型群）を持つ非可換$C^*$代数である。

\textit{(ii) スペクトル作用と$\zeta$。}
$M^4 \times F$上のスペクトル作用は
$\zeta$関数値を生成し~\cite{CCM2007}、BCの
分配関数は$\zeta$関数\emph{そのもの}である。

\textit{(iii) Connes-Marcolliプログラム。}
Connes-Marcolli~\cite{ConnesMarcolli2006}は
BC系を「$\Spec(\mathbb{Z})$上の」量子統計力学の
算術モデルとして明示的に展開している。

これらの構造的論拠に加え、物理的真空がBC構造を持つ\emph{べき}であるという、
より深い\emph{アインシュタイン的}論拠が存在する：

\textit{(iv) $\zeta$正則化の「不合理な有効性」。}
ゼータ関数正則化は発散する和$1+2+3+\cdots$に
値$\zeta(-1) = -1/12$を、$\sum n^3$に$\zeta(-3) = 1/120$を割り当てる。
これらの値をカシミールエネルギー計算に用いた予測は、
実験と$1\%$以内で一致する~\cite{Lamoreaux1997}。
真空が算術構造を持たない「ただの場のモードの集まり」であれば、
数論的な正則化が物理的に正しい結果を出す理由はない。
$\zeta$正則化の経験的成功は、アインシュタインの意味で
構造的説明を要求する「奇跡」である。
最も単純な説明：真空の正しい記述言語が$\zeta(s)$であるのは、
真空の基礎構造が算術的だから。

\textit{(v) オイラー積は選択ではなく必然。}
分解$\zeta(s) = \prod_p(1-p^{-s})^{-1}$は
算術の基本定理（素因数分解の一意性）と同値であり、
$\zeta(s)$の多数ある表現の一つではなく\emph{唯一の}乗法的分解である。
真空エネルギーが$\zeta(-3)$であるならば、それは\emph{既に}素数構造を
持っている --- 我々がそれを課しているのではなく、読み取っているのである。

\textit{(vi) 宇宙定数問題が示唆する診断。}
真空エネルギーの素朴なカットオフ正則化は観測される宇宙定数の
$\sim 10^{120}$倍を与える --- 物理学史上最悪の予測。
$\zeta$正則化は対照的に有限値（$\zeta(-3) = 1/120$）を返す。
「間違った」（カットオフ）正則化が発散し
「正しい」（$\zeta$）正則化が収束するという事実は、
真空が「カットオフ付きの和」ではなく
「$\zeta$関数」としてよく記述されることを示唆する。
そして分配関数が$\zeta$となる自然な量子力学系が、
まさにBC系である。

\textit{(vii) アインシュタインの先例：幾何学を一次的とみなす。}
一般相対論におけるアインシュタインの中心的洞察は、
重力は時空の中で作用する力ではなく時空の幾何学そのものの性質であった。
同様に、$\zeta$正則化は真空エネルギーに適用される計算技法ではなく、
$\Spec(\mathbb{Z})$上の真空の算術幾何学的構造の反映であると提案する。
アインシュタインが「平坦空間中の力」を「空間の曲率」に置き換えたように、
「場のモードの正則化」を「$\Spec(\mathbb{Z})$上の真空の算術構造」に
置き換えるのである。

\textbf{不足しているもの：}
明示的な埋め込み$\mathcal{A}_{\mathrm{BC}} \hookrightarrow \mathcal{A}_F$、
あるいは$\mathcal{A}_{\mathrm{BC}}$上の操作が
$M^4$上のスペクトル作用に特定の修正を誘導することの導出。
上記(iv)--(vii)の論拠は発見法的であり、
予想が成立する\emph{べき理由}を説明するものであって、
成立する\emph{こと}を証明するものではない。
ただし、一般相対論に対するアインシュタイン自身の論拠も、
数学的枠組みが確立されるまでは同様に発見法的（等価原理、マッハの原理）
であったことを指摘しておく。
\cite{paper_A}で提案された実験は、まさにこの問いを検証するために
設計されている。
