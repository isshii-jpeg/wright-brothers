...
Existing demonstrations of Casimir
``repulsion''~\cite{Munday2009,Tang2017} use different mechanisms:
Munday \textit{et al.}\ employed dielectric contrast in a fluid;
Tang \textit{et al.}\ used geometric interplay of T-shaped
nanostructures.  In neither case is the vacuum mode structure
altered---the underlying mode sum remains the full
$\sum_{n=1}^{\infty}n^{-s}$, and no modes are selectively removed.
These experiments demonstrate force-direction control, not vacuum
energy sign reversal.
...
At $s=-1$:
\begin{align}
  \zeta(-1)&=1{+}2{+}3{+}4{+}\cdots=-\tfrac{1}{12},\label{eq:z1}\\
  \zeta_{\neg 2}(-1)&=1{+}3{+}5{+}7{+}\cdots=+\tfrac{1}{12}.\label{eq:z2}
\end{align}
The sign flip $-1/12\to+1/12$ is the
\emph{defining property} of prime-2 muting.

Boyer's result is precisely this:
\begin{itemize}
\item PEC--PEC (DD): modes $n=1,2,3,\ldots$
$\;\Rightarrow\; E\propto\zeta(-1)=-1/12<0$ (attractive).
\item PEC--PMC (DN): modes $n=1,3,5,\ldots$
$\;\Rightarrow\; E\propto\zeta_{\neg 2}(-1)=+1/12>0$ (repulsive).
\end{itemize}
\textbf{Boyer computed $\zeta_{\neg 2}$ without knowing it.}
