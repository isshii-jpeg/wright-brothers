...
\subsection{QED Is Untested in the DN Regime}

A critical point: the standard QED prediction $F\propto a^{-5}$
has been confirmed only for DD boundary conditions.  \textbf{For
DN, this prediction has never been experimentally tested.}
The belief that QED applies equally to DN rests on theoretical
extrapolation, not experimental verification.

DD and DN differ not only in sign but in
\emph{topological structure}:
\begin{equation}
  K_1(\mathbb{Z})=\mathbb{Z}/2 \;\text{(DD)},\qquad
  K_1(\mathbb{Z}[1/2])=\mathbb{Z}/2\times\mathbb{Z}\;\text{(DN)}.
\end{equation}
The DN configuration possesses a winding number ($\mathbb{Z}$
factor) absent in DD.  QED has been tested in the
$K_1=\mathbb{Z}/2$ sector; the $K_1=\mathbb{Z}/2\times\mathbb{Z}$
sector is \emph{terra incognita}.

The precedent for caution is well established: Newtonian gravity
is precisely confirmed at low velocities but fails at high
velocities (special relativity).  A theory's validity in one
topological sector does not guarantee its validity in another.

Consequently, this experiment tests not only Boyer's sign
prediction but also \textbf{whether QED survives its first
encounter with a topologically distinct vacuum sector}.  If
$\alpha=5$ is confirmed for DN, QED gains validation in a new
regime.  If $\alpha\neq 5$, the topological structure of the
vacuum has physical consequences beyond standard QED.

\subsection{What QED Discards and What Might Be Found Inside}

Standard QED treats the divergent vacuum energy
$\sum\hbar\omega_n/2\to\infty$ as an unphysical quantity to be
subtracted by renormalization.  The \emph{regularized} value
(e.g., $\zeta(-3)=1/120$) is used as an intermediate bookkeeping
device; the final physical prediction (the Casimir force) is
obtained by taking \emph{differences} between regularized
energies and is independent of the regularization scheme.

This procedure is spectacularly successful but carries a
hidden assumption: \textbf{the internal structure of the
regularized sum does not matter}.  Whether one uses
$\zeta$-function, dimensional, or cutoff regularization, the
Casimir force comes out the same; the specific value
$\zeta(-3)=1/120$ is considered an artifact, not a feature.

We suggest that this assumption may be too strong.  The
$\zeta$-regularized vacuum energy is not a structureless number:
it is an Euler product,
$\zeta(-3)=\prod_p(1-p^3)^{-1}|_{s=-3}$,
decomposable into prime-indexed factors.  Renormalization
discards the absolute value but preserves differences; it has
never been asked whether the \emph{prime decomposition} of the
discarded value carries physical information.

The DN experiment probes exactly this:
removing the $p{=}2$ factor ($\zeta\to\zeta_{\neg 2}$) changes
the prime structure of the regularized sum.
If the Casimir force is sensitive to this change
\emph{beyond} what standard QED predicts (i.e., if $\alpha\neq 5$),
it would mean that \textbf{the arithmetic structure inside the
renormalized-away vacuum energy was physical all along}---QED
was correct in its predictions but incomplete in its
interpretation of what it was discarding.
