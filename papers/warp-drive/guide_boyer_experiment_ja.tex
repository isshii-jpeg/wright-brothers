...
FIB 加工（大学共同利用、3回） & \textyen 60,000 \\
AFM 使用料（5回） & \textyen 50,000 \\
消耗品（カンチレバー等） & \textyen 10,000 \\
\midrule
\textbf{合計} & \textbf{\textyen 135,000} \\
\bottomrule
\end{tabular}
\end{center}

\begin{honestbox}[Route A の難しさ]
FIB でスロットを開けただけで本当に DN 条件が実現するかは不明。穴の形状・サイズ・周期の最適化が必要。初回で成功する保証はない。複数の設計を試す必要がある → コストが倍増する可能性。
\end{honestbox}
...
nanoVNA（室温テスト用） & \textyen 3,000 \\
SMA ショートキャップ & \textyen 500 \\
トランスモン（共同研究先から借用） & \textyen 0 \\
冷凍機使用（共同研究） & \textyen 0〜100,000 \\
\midrule
\textbf{合計（最小）} & \textbf{\textyen 8,500} \\
\textbf{合計（最大）} & \textbf{\textyen 108,500} \\
\bottomrule
\end{tabular}
\end{center}

\begin{honestbox}[Route B の難しさ]
共同研究先の確保が最大のハードル。量子ビット研究室は忙しく、外部からの提案を受け入れる余裕がないことが多い。Phase 0（室温データ）を持参して「ここまでやった」と示すことが説得の鍵。

また: Lamb シフトの差は「ζ 正則化の検証」ではなく「有限個のモードの足し引き」。スケーリング（α の値）は直接測定できない。Route B は「符号反転の間接的証拠」であり、Route A/C の「力の直接測定」に比べて弱い。
\end{honestbox}
...
クリーンルーム使用料（10回） & \textyen 200,000 \\
フォトマスク製作 & \textyen 50,000 \\
薬品・消耗品 & \textyen 20,000 \\
\midrule
\textbf{合計} & \textbf{\textyen 300,000} \\
\bottomrule
\end{tabular}
\end{center}

\begin{honestbox}[Route C の難しさ]
MEMS 製作はクリーンルーム経験が必要。Tang の論文の著者と同レベルの技術が求められる。個人で始めるより、MEMS の研究室との共同研究が現実的。
...
\midrule
温度 & 室温 & 10 mK & 室温 \\
コスト & $\sim$14万 & 1〜11万 & $\sim$30万 \\
力の直接測定 & ✓ & ✗（間接） & ✓ \\
$\alpha$ 測定 & ✓ & ✗ & ✓ \\
個人で製作 & △ & △（共振器のみ）& ✗ \\
共同研究必要 & △（FIB） & ✓（冷凍機）& ✓（クリーンルーム）\\
DD対照実験 & ✓ & ✓ & ✓（同一チップ）\\
\midrule
推奨度 & ★★ & ★★ & ★★★ \\
\bottomrule
...
\textbf{Phase 0}（\textyen 8,500、個人）: Route B の $\lambda/4$ vs $\lambda/2$ 室温テスト。偶奇分離のデモ。

\textbf{Phase 1}（\textyen 14万、共同研究）: Route A でナノ DN + AFM。Boyer 斥力の初検出。

\textbf{Phase 2}（\textyen 30万、共同研究）: Route C で MEMS スケーリング測定。$\alpha$ の精密決定。位相的転移の有無の判定。
\end{resultbox}
